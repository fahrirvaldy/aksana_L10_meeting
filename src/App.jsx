...
const handleDeleteAttendance = (id) => {
  ...
  // Langsung hapus rating berdasarkan ID agar Firebase tidak menyimpan "sampah"
...
const handleUpdateAttendance = (id, field, value) => {
  ...
  // Jika uncheck (value === false), otomatis hapus ratingnya
        ...
        <div key={sheet.key} className={`slide ${currentSlide === (index + 2) ? 'active' : ''}`}>
          ...
          <div className="flex flex-col gap-1 mb-4">
            ...
            <h1 className="flex items-center gap-2">
              ...
              // KUNCI: Ganti || menjadi ?? (Nullish Coalescing)

                value={data.scorecardTitles?.[sheet.key] ?? sheet.title} 

                onChange={(val) => updateData(`scorecardTitles.${sheet.key}`, val)} 

                className="text-3xl md:text-5xl font-black text-aksana-primary p-0 m-0 leading-none w-full bg-transparent"
          ...
          <div className="card" style={{ height: 'auto' }}>
                ...
                <tbody>
                  ...
                  {data[sheet.key].map((item, i) => (
                    ...
                    <tr key={i} className="group">
                      ...
                      <td align="center"><JenisButton jenis={item.jenis} onClick={() => updateListItem(sheet.key, i, 'jenis', cycleJenis(item.jenis))} /></td>
            ...
            <button className="add-btn" onClick={() => addRow(sheet.key, { kpi: '', target: '', real: '', jenis: 'leading', status: 'on' })}>+ Tambah KPI</button>
...
<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 my-8">
  ...
  {attendances.filter(a => a.checked).map((attendee) => {

    // KUNCI UTAMA: Gunakan ID agar Firebase tidak error

    const roleKey = `rating_id_${attendee.id}`; 

    return (

      <div key={attendee.id} className="flex flex-col items-center bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:border-aksana-primary transition-all">

        <label className="capitalize text-[10px] font-bold text-slate-400 mb-2 tracking-widest text-center">
        ...
        <input 
          ...
          onChange={(e) => {
            ...
            // Gunakan updateData yang sudah ada
